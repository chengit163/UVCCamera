...
public final class DeviceFilter
{
    ...
    public DeviceFilter(final int vid, final int pid, final int clasz,
                        final int subclass, final int protocol, final String manufacturer,
                        final String product, final String serialnum)
    {
        mVendorId = vid;
        mProductId = pid;
        mClass = clasz;
    ...
    {
        final XmlPullParser parser = context.getResources().getXml(
                deviceFilterXmlId);
     ...
     * @return
     */
    private static final int getAttributeInteger(final Context context,
                                                 final XmlPullParser parser, final String namespace,
    ...
    /**
     * read as String attribute with default value from xml(w/o exception
     * throws) resource string id is also resolved into string
     *
     * @param parser
     * @param namespace
     * @param name
    ...
    public static DeviceFilter read(final Context context,
            ...
            IOException
    {
        ...
        while (eventType != XmlPullParser.END_DOCUMENT)
        {
            ...
            if (!TextUtils.isEmpty(tag) && (tag.equalsIgnoreCase("usb-device")))
            {
                ...
                {
                    hasValue = true;
                    vendorId = getAttributeInteger(context, parser, null,
                            "vendor-id", -1);
                    if (vendorId == -1)
                    {
                        vendorId = getAttributeInteger(context, parser, null,
                                "vendorId", -1);
                        if (vendorId == -1)
                            vendorId = getAttributeInteger(context, parser,
                                    null, "venderId", -1);
                    }
                    productId = getAttributeInteger(context, parser, null,
                            "product-id", -1);
                    if (productId == -1)
                        productId = getAttributeInteger(context, parser, null,
                                "productId", -1);
                    deviceClass = getAttributeInteger(context, parser, null,
                            "class", -1);
                    deviceSubclass = getAttributeInteger(context, parser, null,
                            "subclass", -1);
                    deviceProtocol = getAttributeInteger(context, parser, null,
                            "protocol", -1);
                    manufacturerName = getAttributeString(context, parser,
                            null, "manufacturer-name", null);
                    if (TextUtils.isEmpty(manufacturerName))
                        manufacturerName = getAttributeString(context, parser,
                                null, "manufacture", null);
                    productName = getAttributeString(context, parser, null,
                            "product-name", null);
                    if (TextUtils.isEmpty(productName))
                        productName = getAttributeString(context, parser, null,
                                "product", null);
                    serialNumber = getAttributeString(context, parser, null,
                            "serial-number", null);
                    if (TextUtils.isEmpty(serialNumber))
                        serialNumber = getAttributeString(context, parser,
                                null, "serial", null);
     ...
     * serializer.startTag(null, "usb-device"); if (mVendorId != -1) {
     * serializer .attribute(null, "vendor-id", Integer.toString(mVendorId)); }
     * if (mProductId != -1) { serializer.attribute(null, "product-id",
     * Integer.toString(mProductId)); } if (mClass != -1) {
     * serializer.attribute(null, "class", Integer.toString(mClass)); } if
     * (mSubclass != -1) { serializer.attribute(null, "subclass",
     * Integer.toString(mSubclass)); } if (mProtocol != -1) {
     * serializer.attribute(null, "protocol", Integer.toString(mProtocol)); } if
     * (mManufacturerName != null) { serializer.attribute(null,
     * "manufacturer-name", mManufacturerName); } if (mProductName != null) {
     * serializer.attribute(null, "product-name", mProductName); } if
     * (mSerialNumber != null) { serializer.attribute(null, "serial-number",
     * mSerialNumber); } serializer.endTag(null, "usb-device"); }
     */

    private boolean matches(final int clasz, final int subclass,
                            final int protocol)
    {
    ...
    public boolean matches(final UsbDevice device)
    {
        ...
        if (mProductId != -1 && device.getProductId() != mProductId)
            return false;
        /*
         * if (mManufacturerName != null && device.getManufacturerName() ==
         * null) return false; if (mProductName != null &&
         * device.getProductName() == null) return false; if (mSerialNumber !=
         * null && device.getSerialNumber() == null) return false; if
         * (mManufacturerName != null && device.getManufacturerName() != null &&
         * !mManufacturerName.equals(device.getManufacturerName())) return
         * false; if (mProductName != null && device.getProductName() != null &&
         * !mProductName.equals(device.getProductName())) return false; if
    ...
    public boolean equals(final Object obj)
    {
        ...
        if (obj instanceof UsbDevice)
        {
            ...
            /*
             * if ((mManufacturerName != null && device.getManufacturerName() ==
             * null) || (mManufacturerName == null && device
             * .getManufacturerName() != null) || (mProductName != null &&
             * device.getProductName() == null) || (mProductName == null &&
             * device.getProductName() != null) || (mSerialNumber != null &&
             * device.getSerialNumber() == null) || (mSerialNumber == null &&
             * device.getSerialNumber() != null)) { return (false); }
             */
            /*
             ...
             * .equals(device.getProductName())) || (device.getSerialNumber() !=
             * null && !mSerialNumber .equals(device.getSerialNumber()))) {
             * return (false); }
             */
            return true;
        }
...
//				+ ",mProtocol=" + mProtocol + ",mManufacturerName="
//				+ mManufacturerName + ",mProductName=" + mProductName
