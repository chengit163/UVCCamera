...
public final class USBMonitor
{
    ...
    private static final String ACTION_USB_PERMISSION_BASE = "com.cit.uvccamera.USB_PERMISSION.";
...
/*		if (listener == null)
     ...
     *
     * @param device 设备
     * @param isMust 必须获取到权限(true:请求失败会弹授权框，false:不了了之)
     */
    public synchronized void requestPermission(final UsbDevice device, boolean isMust)
    {
        if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
        if (mPermissionIntent != null)
        {
            if (device != null)
            {
                ...
                {
                    // mUsbManager.requestPermission(device, mPermissionIntent);
                    tryGrantDevicePermission(device, isMust);
                }
            } else
            {
                processCancel(device);
    ...
    /**
     * 尝试直接取得权限，具备系统签名的app能执行成功
     *
     * @param device 设备
     * @param isMust 必须获取到权限
     */
    private void tryGrantDevicePermission(final UsbDevice device, boolean isMust)
    {
        final Context context = mWeakContext.get();
        if (context != null)
        {
            try
            {
                ...
                IBinder binder = (IBinder) method.invoke(serviceManager.newInstance(), Context.USB_SERVICE);
                //
            ...
            } catch (Exception e)
            {
                ...
                if (isMust) mUsbManager.requestPermission(device, mPermissionIntent);
            }
        } else
        {
            Log.w(TAG, "context denied");
            if (isMust) mUsbManager.requestPermission(device, mPermissionIntent);
        }
    }
